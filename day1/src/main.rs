...
fn parse(text: &str) {
    ...
    for mut line in text.split_whitespace().map(|txt| txt.to_string()) {
        match (line.remove(0), line) {
            ('L', n) => dail.turn_dial(-n.parse::<i32>().unwrap()),
            ('R', n) => dail.turn_dial(n.parse::<i32>().unwrap()),
            (_, _) => continue,
        }
        println!("--------------------------------");

        println!("Currently at: {}", dail.current_num);
    }
    println!("N times at 0: {}", dail.n_times_at_0);
}
